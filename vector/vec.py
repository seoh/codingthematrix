...
def equal(u,v):
    ...
    """
    return u.D == v.D and (
        sorted([kv for kv in u.f.items() if kv[1]]) ==
        sorted([kv for kv in v.f.items() if kv[1]]))
...
def add(u,v):
    ...
    """
    if u.D == v.D:
        return Vec(u.D, {k: (u.f[k] if k in u.f else 0) + (v.f[k] if k in v.f else 0)
                  for k in set(u.f.keys()).union(set(v.f.keys()))})
    else:
        return u
...
def dot(u,v):
    ...
    """
    if u.D == v.D:
        return sum([u.f[k]*v.f[k] for k
                    in set(u.f.keys()).intersection(set(v.f.keys()))])
    else:
        return u
